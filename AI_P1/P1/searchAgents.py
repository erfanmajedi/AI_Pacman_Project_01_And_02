...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        cur_pos = state[0]
        founded = state[1] # founded corners
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # Here's a code snippet for figuring out whether a new position hits a wall:
              ...
              if not hitsWall:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in founded:
                    explored = founded + [(nextx, nexty)]
                    successors.append((((nextx, nexty), explored), action, 1))
                else:
                    successors.append((((nextx, nexty), founded), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    # walls = problem.walls 
    not_explored = []        
    explored = state[1] # the corners which are explored 
    node = state[0]  # node = current state 
    heuristic = 0       
    # Find all the corners that we haven't visited yet, and append them to a 
    # list so we can go through them
    for corner in corners:
        if not corner in explored :
            not_explored.append(corner)
    
    # Find the sum of the shortest distances between the not_explored corners. Use 
...
def foodHeuristic(state, problem):
    ...
    distance = 0
    for dot in dots:
        # because of that 5 is the smallest bound that autograde gives to us we used 5. now if it becomes more than 5 we
